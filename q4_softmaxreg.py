...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sentence_ind = [tokens[w] for w in sentence]
    sentVector = np.mean(wordVectors[sentence_ind,:], axis=0)
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    ds = prob
    ds[range(N),labels] -= 1.0
    grad = np.dot(features.transpose(),ds) / N + regularization*weights
